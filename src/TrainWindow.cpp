	...
	//========================================================================
{
	...
	begin();	// add to this widget
	{
		...
		addTrain = new Fl_Button(605, pty, 80, 20, "Add Train");
		//addTrain->callback((Fl_Callback*)addTrainCB, this);
		deleteTrain = new Fl_Button(690, pty, 80, 20, "Delete Train");
		//deleteTrain->callback((Fl_Callback*)deleteTrainCB, this);
		pty += 30;

		lightOn = new Fl_Button(605, pty, 80, 20, "Light On");
		togglify(lightOn, 1);

		pty += 30;
		scale = new Fl_Value_Slider(655, pty, 140, 20, "scale");
		scale->range(0, 1);
		scale->value(0.5);
...
//========================================================================
{
	static int orType = CARDINAL;

	if (trainView->tw->splineBrowser->value() != orType)
	{
		trainView->change = true;
		orType = trainView->tw->splineBrowser->value();
	}
...
//========================================================================
{
	...
	{
		genTable(trainView);
		trainView->change = false;
	}
	if (trainView->tw->arcLength->value() == false)
	{
		trainView->t_time += (dir / m_Track.points.size() / (trainView->DIVIDE_LINE / 40)) * trainView->tw->speed->value();
		if (trainView->t_time > 1.0f)
		{
			trainView->t_time -= 1.0f;

			trainView->countTrain++;
			if (trainView->countTrain >= trainView->m_pTrack->points.size())
				trainView->countTrain = 0;
		}
	}
	else
	{
		...
		trainView->d_distance += 1.0 * trainView->tw->speed->value();
		if (trainView->d_distance > trainView->totalDistant)
		{
			trainView->d_distance = 0.0f;
		}
	}

	trainView->water_time += 0.1;
